...
numbersList = []
while True:
    newNuber = input("Введите число или stop для завершения работы программы")
    if newNuber == "stop":
        break
    try:
        if not newNuber.isdigit():
            raise NotNumberException()
    except NotNumberException as exception:
        print(exception)
    else:
        numbersList.append(int(newNuber))
print(f"Результат: {numbersList}")
